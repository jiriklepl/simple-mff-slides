...
\begin{frame}{How we achieve these results}
  \begin{itemize}
    \only<1,2>{\item[\color{blue!66}\faArrowRight] Parametric polymorphism with definable type constructors}
    \only<2> {
      \begin{itemize}
    ...
    \only<1,3>{\item[\color{blue!66}\faArrowRight] Multi-parameter typeclasses: overloading constrained by a relation}
    \only<3>{
      \begin{itemize}
    ...
    \only<1,4>{\item[\color{blue!66}\faArrowRight] Automatic calls to resource management functions (bound to a resource lifetime) }
    \only<4>{
      \begin{itemize}
    ...
    \only<1,5>{\item[\color{blue!66}\faArrowRight] New subtype extension to Hindley-Milner-style type systems }
    \only<5>{
      \begin{itemize}
